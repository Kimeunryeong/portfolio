$(window).load(function () {
  ...
  let skills1 = $("#skills1"); //고정 시작 영역
  let leftBox = $("#left_box"); // 고정될 영역
  let nextSection = $("#work"); //반 스크롤 다음 섹션
  let titLi = $(tit_list).children(); // 제목 리스트 Li
  let titLiNum = titLi.size(); //제목 리스트 Li 개수
  let txtLi = $(txt_list).children(); // 텍스트 리스트 Li
  let idx = 0;
  let windowWidth
